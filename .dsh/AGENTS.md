...
3. **数据与包获取三层状态机与思维链 (3-Tier Acquisition State Machine & CoT — 强行绑定云服务器环境)**:
   - **执行范围声明**：**本地项目不设此思维链，本套状态机与思维链强行绑定云服务器/容器环境（Cloud Server Environment）**。
   - **云端思考链必填状态推导**: 在云端寻找任何数据前，必须按 `[Tier 1 Check: Google Drive]` -> 若未命中输出 `[Tier 1 Miss -> Escalating to Tier 2]` -> `[Tier 2 Check: Relay/Cluster RAG]` -> 若未命中输出 `[Tier 2 Miss -> Tier 3 Fallback]` 格式推导。
...
7. **标准化自动补齐闭环执行链路与严禁消极阻断 (Strict Ban on Passive Blocking & Mandatory Autonomous Resolution — 云服务器环境专属)**:
   - **适用环境**：**本地项目不设此思维链，本套思维链与闭环链路强行绑定云服务器/容器环境**。
