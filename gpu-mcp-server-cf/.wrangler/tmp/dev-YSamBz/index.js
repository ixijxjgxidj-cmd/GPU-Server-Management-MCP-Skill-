...
// .wrangler/tmp/bundle-Q0Yirm/strip-cf-connecting-ip-header.js
...
var listServersTool = {
  ...
  execute: async (args, { db }) => {
    const tag = args.tag;
    const servers = await listServers(db, tag, true);
...
var addServerTool = {
  ...
  execute: async (args, { db }) => {
    const id = await createServer(db, {
      ...
      v2ray_available: args.v2ray_available ? 1 : 0,
      direct_when_proxy_available: args.direct_when_proxy_available ? 1 : 0,
      direct_when_no_proxy: args.direct_when_no_proxy ? 1 : 0,
...
var updateServerTool = {
  ...
  execute: async (args, { db }) => {
    const success = await updateServer(db, args.server_id, args.updates);
...
// src/mcp/tools/query_servers.ts
var queryServersTool = {
  definition: {
  ...
  execute: async (args, { db }) => {
    const servers = await queryServersByAbility(db, {
      ...
      status_online: args.status_online
    });
    const summaries = servers.map((s) => ({
      id: s.id,
      name: s.name,
...
var recordUsageTool = {
  ...
  execute: async (args, { db }) => {
    const id = await recordUsage(db, {
      ...
      action: args.action,
      details: args.details ? JSON.stringify(args.details) : void 0
    });
    return { content: [{ type: "text", text: JSON.stringify({ id }) }] };
  }
};
...
async function grabSSHBanner(host, port, timeoutMs = 5e3) {
  ...
  let socket;
  try {
    socket = connect2({ hostname: host, port });
    await socket.opened;
    const reader = socket.readable.getReader();
    const timer = setTimeout(() => {
      try {
...
var testProxyTool = {
  ...
  execute: async (args, { db }) => {
    const server = await getServerById(db, args.server_id);
    if (!server) {
      return { content: [{ type: "text", text: "Server not found" }], isError: true };
    }
    const proxy = await getProxyById(db, args.proxy_id);
    if (!proxy) {
...
// src/mcp/tools/find_best_server.ts
var findBestServerTool = {
  definition: {
  ...
  execute: async (args, { db }) => {
    ...
    const minDiskGb = args.min_disk_gb;
    ...
    const servers = await queryServersByAbility(db, {
      ...
      status_online: requireOnline ? true : void 0
    });
...
// src/mcp/tools/get_cluster_summary.ts
var getClusterSummaryTool = {
  definition: {
  ...
  execute: async (args, { db }) => {
    const servers = await listServers(db, void 0, true);
    const proxies = await listProxies(db);
    ...
    for (const s of servers) {
      ...
      if (s.gpu_memory_gb)
        totalGpuMemory += s.gpu_memory_gb;
      if (s.ram_gb)
        totalRam += s.ram_gb;
      if (s.cpu_cores)
        totalCpuCores += s.cpu_cores;
    }
...
// src/mcp/tools/batch_query_servers.ts
var batchQueryServersTool = {
  definition: {
  ...
  execute: async (args, { db }) => {
    ...
    if (groupByConnectivity) {
      const directGroup = enriched.filter(
        (s) => s.connection_mode.direct_when_no_proxy || s.connection_mode.v2ray_available && s.connection_mode.direct_when_proxy_available
      );
      const proxyGroup = enriched.filter(
        (s) => !s.connection_mode.direct_when_no_proxy && !(s.connection_mode.v2ray_available && s.connection_mode.direct_when_proxy_available)
      );
      return {
        content: [{
...
// src/mcp/tools/batch_assign_servers.ts
var batchAssignServersTool = {
  definition: {
  ...
  execute: async (args, { db }) => {
    ...
    const appendTags = args.append_tags;
    ...
    for (const serverId of serverIds) {
      try {
        ...
        if (server.tags) {
          try {
            currentTags = JSON.parse(server.tags);
          } catch {
            currentTags = [];
          }
        }
        ...
        if (updates) {
          if (updates.default_proxy_id !== void 0)
            effectiveUpdates.default_proxy_id = updates.default_proxy_id;
          if (updates.v2ray_available !== void 0)
            effectiveUpdates.v2ray_available = updates.v2ray_available ? 1 : 0;
          if (updates.direct_when_proxy_available !== void 0)
            effectiveUpdates.direct_when_proxy_available = updates.direct_when_proxy_available ? 1 : 0;
          if (updates.direct_when_no_proxy !== void 0)
            effectiveUpdates.direct_when_no_proxy = updates.direct_when_no_proxy ? 1 : 0;
        }
...
// src/mcp/tools/batch_probe_servers.ts
async function probeSingleServer(serverId, host, port) {
  const result = await tcpPing(host, port);
  return { server_id: serverId, ...result };
}
__name(probeSingleServer, "probeSingleServer");
var batchProbeServersTool = {
  definition: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        timeout_ms: { type: "number", default: 3e3, description: "\u5355\u53F0\u8D85\u65F6\u65F6\u95F4\uFF08\u6BEB\u79D2\uFF09\u3002\u9ED8\u8BA4\u4E3A3000\uFF083\u79D2\uFF09\u3002" }
      }
    }
  },
  execute: async (args, { db }) => {
    ...
    const updatePromises = probeResults.map(
      (r) => updateServerStatus(db, r.server_id, {
        online: r.reachable,
...
// src/mcp/tools/release_server.ts
var releaseServerTool = {
  definition: {
    name: "release_server",
  ...
  execute: async (args, { db }) => {
    ...
    const result = {
      ...
      task_done: taskDone,
      message: wasBusy ? `\u2705 ${server.name} \u5DF2\u91CA\u653E\uFF08${previousAgent} \u7684 "${previousTask}"\uFF09` : `\u2705 ${server.name} \u5DF2\u7ECF\u662F\u7A7A\u95F2\u72B6\u6001`
    };
    if (note)
      result.note = note;
    return {
...
// src/mcp/tools/enable_server.ts
var enableServerTool = {
  definition: {
    name: "enable_server",
...
var disableServerTool = {
  ...
  execute: async (args, { db }) => {
    await setServerEnabled(db, args.server_id, false);
    return { content: [{ type: "text", text: JSON.stringify({ success: true, enabled: false }) }] };
  }
};

// src/mcp/tools/detect_hardware.ts
var detectHardwareTool = {
  definition: {
  ...
  execute: async (args, { db }) => {
    ...
    const updates = {};
    if (args.gpu_model)
      updates.gpu_model = args.gpu_model;
    if (args.gpu_memory_gb !== void 0)
      updates.gpu_memory_gb = args.gpu_memory_gb;
    if (args.cpu_cores !== void 0)
      updates.cpu_cores = args.cpu_cores;
    if (args.ram_gb !== void 0)
      updates.ram_gb = args.ram_gb;
    if (args.disk_gb !== void 0)
      updates.disk_gb = args.disk_gb;
    if (!args.gpu_model && args.gpu_count) {
      const gpuCount = args.gpu_count;
      if (gpuCount > 0)
        updates.gpu_model = `${gpuCount}x GPU`;
    }
...
// .wrangler/tmp/bundle-Q0Yirm/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Q0Yirm/middleware-loader.entry.ts
